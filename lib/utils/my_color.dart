// ignore_for_file: prefer_const_constructors

import 'package:flutter/material.dart';

class MyColors {
  static Color white = Colors.white;
  static Color black = Colors.black;
  static Color blackGrey = Color(0xff313131);
  static Color orange = Color(0xffE53D00);
  static Color blue = Color(0xff0A65A5);
  static Color green = Color(0xff59C23A);
  static Color brown = Color(0xffB12D2D);
  static Color lightIndigo = Color(0xffF0F3FF);
  static Color lightRed = Color(0xffFFEBEB);
  static Color grey = Color(0xff999999);
  static Color lightGreen = Color(0xffEBFFF4);
  static Color border = Color(0xffE0E0E0);
  static Color indigo = Color(0xffB6BFCD);
  static Color lightGrey = Color(0xffF0F0F0);
}
